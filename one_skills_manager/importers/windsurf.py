"""Import existing Windsurf configuration."""
...
def import_mcp_servers(source_path: Path, mcp_config: MCPConfig) -> list[str]:
    """Import MCP servers from Windsurf's mcp_config.json."""
    ...
    for server_name, server_data in mcp_servers.items():
        ...
        if "serverUrl" in server_data:
            # SSE transport
            transport_name = "sse"
            transport = MCPTransport(
                type="sse",
                url=server_data["serverUrl"],
        ...
        server = MCPServer(
            name=server_name,
            description=f"Imported from Windsurf ({transport_name})",
...
def import_rules(source_file: Path, dest_dir: Path) -> list[str]:
    """Import Windsurf's global_rules.md file."""
    if not source_file.exists():
        return []

    dest_file = dest_dir / "windsurf-global-rules.md"
    ...
    return ["windsurf-global-rules.md"]


def import_skills(source_dir: Path, config) -> list[str]:
    """Import skills from Windsurf's ~/.codeium/windsurf/skills/."""
